...
class DeepPolicyInferQLearner(DeepQLearner): 
    ...
    def _build_optimize_op(self):
        ...
        opponent_action = self.mode.get_inputs()['opponent_action']

        # Outputs
        policy_q = self.model.get_outputs()['policy_q']
        target_q = self.model.get_outputs()['target_q']
        opponent_action_logit = self.model.get_outputs()['opponent_action_logit']
        ...
        # Auxiliary loss
        opponent_action_cross_entropy = tf.nn.softmax_cross_entropy_with_logits(labels=opponent_action, logits=opponent_action_logit)
        
        loss = tf.reduce_mean(HuberLoss(target - pred) + opponent_action_cross_entropy, name='loss')

...
class DeepQNetwork(Model):
    ...
    def get_target_network_vars(self):
        assert self.with_target_network
        return tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='%s/target' % self.name)
           
class DeepRecurrentQNetwork(DeepQNetwork):
    def __init__(self, name, reuse, state_shape, num_action, max_trace, with_target_network=True):
        super(DeepRecurrentQNetwork, self).__init__(name, reuse, state_shape, num_action, with_target_network)

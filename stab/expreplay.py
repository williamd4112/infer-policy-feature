...
class ExpReplay(DataFlow, Callback):
    ...
    def __init__(self,
                 ...
                 init_exploration,
                 update_frequency, history_len, repop=False):
        """
        Args:
    ...
    def _populate_exp(self):
        ...
        self.mem.append(Experience(old_s, act[:2], reward, isOver))
    ...
    def _trigger_epoch(self):
        ...
        for k, v in six.iteritems(stats):
            try:
                v = np.array(v)
                #self.trainer.monitors.put_scalar('expreplay/episode', self.episode)
                self.trainer.monitors.put_scalar('expreplay/episode', self.n_epoch)
                self.trainer.monitors.put_scalar('expreplay/eps', self.exploration)
        ...
        if not self.repop and len(self.mem) > 1.02e6:
            print("repopulating")
            self.repop = True
            self.mem.reset()
            self._init_memory()

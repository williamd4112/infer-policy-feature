...
def get_rnn_cell():
    return tf.nn.rnn_cell.LSTMCell(num_units=512, state_is_tuple=True, activation=tf.nn.relu)
    #if RNN_CELL == 'gru':
...
class MAModel(ModelDesc):
    def __init__(self, image_shape, channel, method, num_actions, gamma, ctrl_size=1,
                lr=1e-3, lamb=1.0, h_size=512, update_step=1, multi_task=False, num_agents=3,
                reg=True, mt_type='all', models=None, dnp=False, ent=False, thresh=None, stab=None, use_rnn=True, pi_agents=1):
        ...
        self.dnp = dnp
        self.ent = ent
        self.lamb = symbf.get_scalar_var('lamb', 1.0, summary=True)
        self.thresh = thresh
        self.pi_agents = pi_agents
        ...
        if stab == 'both':
            ...
            stab = [True, False]
        print("stab --- {}".format(stab))

        if self.models_type == 'both-dqn':
            print("both dqn")
            self.models = [DQNModel(rank=0, image_shape=image_shape, channel=channel, stab=stab[0], num_actions=num_actions,
                           method=method, num_agents=num_agents, dnp=self.dnp, use_rnn=self.use_rnn),
                           DQNModel(rank=1, image_shape=image_shape, channel=channel, stab=stab[1], num_actions=num_actions,
                           method=method, num_agents=num_agents, dnp=self.dnp, use_rnn=self.use_rnn)]
        else:
            print("one pi")
            self.models = [PIModel(rank=0, image_shape=image_shape, channel=channel, stab=stab[0], method=method,
                           num_actions=num_actions, num_agents=pi_agents, use_rnn=self.use_rnn),
                           DQNModel(rank=1, image_shape=image_shape, channel=channel, stab=stab[1], num_actions=num_actions,
                           method=method, num_agents=num_agents, dnp=self.dnp, use_rnn=self.use_rnn)]
    ...
    def _get_inputs(self):
        ...
        return [InputDesc(tf.uint8,
                ...
                InputDesc(tf.float32, (None, self.channel + 1), 'eps'),
                InputDesc(tf.int64, (None, self.channel + 1), 'episode'),
                ]
    ...
    def _get_DQN_prediction(self, image, eps, epis, models):
        qs, pis = [], []
        for i, model in enumerate(models):
            with tf.variable_scope("network-%d" % (i)):
                q, pi, _, _ = model._get_DQN_prediction(image, eps, epis)
            qs.append(q)
            pis.append(pi)
    ...
    def _build_graph(self, inputs):
        comb_state, action, reward, isOver, action_o, eps, episode = inputs
        ...
        #eps = tf.slice(eps, [0, backward_offset], [-1, self.update_step])
        #episode = tf.slice(episode, [0, backward_offset], [-1, self.update_step])
        ...
        predict_value, pi_value, _, _ = self._get_DQN_prediction(state, eps[:, self.channel-1], episode[:, self.channel-1], self.models)
        ...
        with tf.variable_scope('target'), \
            ...
            targetQ_pred_value, target_pi_value, _, _ = self._get_DQN_prediction(next_state, eps[:, self.channel], episode[:, self.channel], self.models)
        ...
        if self.method != 'Double':
            ...
            with tf.variable_scope(sc, reuse=True):
                next_predict_value, _, _, _ = self._get_DQN_prediction(next_state, eps[:, self.channel], episode[:, self.channel], self.models)
        ...
        for i, o in enumerate(action_o_one_hots):
            ...
            x_ent_costs.append(tf.reduce_sum(pi_prob*log_prob) * 0.01)


        q_cost = tf.reshape((symbf.huber_loss(target - pred_action_value)), (self.batch_size * self.update_step, self.ctrl_size))
        self.cost_pi = q_cost[:, 0]

        if self.models_type == 'both' or self.models_type == 'pi':
            print("adopt pi_loss")
            pi_cost = tf.reduce_mean(tf.add_n(pi_costs))
            x_ent_cost = tf.reduce_mean(tf.add_n(x_ent_costs))
            if self.thresh is not None:
                pi_cost = tf.cond(pi_cost > self.thresh, true_fn=lambda: pi_cost, false_fn=lambda:pi_cost * 0.)
            summary.add_moving_summary(tf.reduce_mean(pi_cost, name='pi_loss'))
            ...
            self.cost_pi = q_cost[:, 0] * reg_coef + self.lamb * pi_cost
            if self.ent:
                print('apply xent_loss')
                self.cost_pi += x_ent_cost
                summary.add_moving_summary(tf.reduce_mean(x_ent_cost, name='xent_loss'))

        self.cost_pi = tf.reduce_mean(self.cost_pi, name='cost_pi')
        self.cost_dqn = tf.reduce_mean(q_cost[:, 1], name='cost_dqn')

        if self.models_type == 'both' or self.models_type == 'both-dqn':
...
class DQNModel(MAModel):
    def __init__(self, rank, num_actions, h_size=512, dnp=False, stab=False, **kwargs):
    ...
    def _get_DQN_prediction(self, image, eps, epis):
        """ image: [0,255]"""
        image = image / 255.0
        self.batch_size = tf.shape(image)[0]
        eps = tf.expand_dims(eps, axis=1)
        epis = tf.expand_dims(epis, axis=1)
        ...
        with tf.variable_scope('q'):
            with argscope(Conv2D, nl=PReLU.symbolic_function, use_bias=True, padding='SAME'), \
                ...
                """
                if self.stab:
                    h = tf.reshape(h, [self.batch_size, 11*11*64])
                    h = tf.concat([h, tf.to_float(epis), eps], axis=-1)
                    print("stab {}".format(self.rank))
                """
                ...
                if self.use_rnn:
                    ...
                    q_l = tf.reshape(q_l, (self.batch_size * self.rnn_step, self.rnn_hidden))
        ...
        if self.stab:
            l = tf.concat([l, tf.to_float(epis), eps], axis=-1)
...
class PIModel(MAModel):
    def __init__(self, rank, use_rnn=False, rnn_hidden=512, rnn_step=1, h_size=512, num_agents=1, stab=False, **kwargs):
        self.rank = rank

...
class SoccerPlayer(RLEnvironment):
    ...
    def __init__(self, viz=0,
                ...
                image_shape=(84, 84),
                mode=None, team_size=4, ai_frame_skip=1, train_type='both', raw_env=soccer_environment.SoccerEnvironment):
        ...
        if self.field == 'large' :
            map_path = file_util.resolve_path(__file__, '../data/map/soccer_21x14_goal_4.tmx')
    ...
    def finish_episode(self):
        ...
        for i in range(self.team_size*2):
            if self.env.state.is_agent_win(i):
                self.stats['scorer'].append(i)
    ...
    def action(self, act):
        ...
        for k in range(self.frame_skip):
            ...
            if self.mode[0] == 'DQN_OPPONENT' or type(act) is list:
                actions = {i: None for i in range(self.env.options.team_size*2)}
                """
                if self.train_type == 'both':
                    for i, team_name in enumerate(self.env.team_names):
                        agent_index = self.env.get_agent_index(team_name, 0)
                        actions[agent_index] = self.env.actions[act[i]]
                elif self.train_type == 'dqn':
                    actions[0] = None
                    actions[self.env.options.team_size] = self.env.actions[act[1]]
                else:
                    actions[0] = self.env.actions[act[0]]
                    actions[self.env.options.team_size] = None
                """
                if self.train_type == 'both' or self.train_type == 'both-dqn':
                    actions[0] = self.env.actions[act[0]]
                    #actions[0] = 'STAND'

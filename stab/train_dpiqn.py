...
def get_config():
    if TASK == 'play':
        if MULTI_TASK:
            predictor_io_names=(['state', 'eps', 'episode'], ['network-0/Qvalue-0', 'network-1/Qvalue-1', 'network-0/Pivalue-0-0', 'network-0/Pivalue-0-1'])
        else:
            predictor_io_names=(['state', 'eps', 'episode'], ['network-0/Qvalue-0', 'network-1/Qvaule-1', 'network-0/Pivalue-0-0'])
    else:
        predictor_io_names=(['state', 'eps', 'episode'], ['network-0/Qvalue-0', 'network-1/Qvalue-1'])

    M = MAModel(image_shape=IMAGE_SIZE, channel=FRAME_HISTORY, method=METHOD, num_actions=NUM_ACTIONS,
            gamma=GAMMA, ctrl_size=CTRL_SIZE, lamb=LAMB, reg=REG, models=MODELS, dnp=DNP, ent=ENT, thresh=THRESHOLD, pi_agents=PIA, use_rnn=USE_RNN, stab=STAB)
    ...
    return TrainConfig(
        ...
        callbacks=[
            ...
            expreplay,
            #ScheduledHyperParamSetter('lamb',
            #                              [(100, LAMB)]),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--threshold', help='', default=None)
    ...
    parser.add_argument('--type', help='algorithm for computing Q-value',
                        choices=['both', 'both-dqn', 'pi', 'dqn'], default='both')
    parser.add_argument('--mode', help='specify ai mode in env', type=str, default=None)
    parser.add_argument('--stab', help='specify ai mode in env', type=str, default=None)
    ...
    parser.add_argument('--pia', help='act repeat', type=int, default=1)
    ...
    parser.add_argument('--repop', help='repop', action='store_true', default=False)
    ...
    if args.task != 'train':
        ...
        model = MAModel(image_shape=IMAGE_SIZE, channel=FRAME_HISTORY, method=METHOD, num_actions=NUM_ACTIONS,
            gamma=GAMMA, ctrl_size=CTRL_SIZE, lamb=LAMB, reg=REG, models=MODELS, dnp=DNP, ent=ENT, thresh=THRESHOLD, pi_agents=PIA, use_rnn=USE_RNN, stab=STAB)

        if MODELS == 'both' or MODELS == 'both-dqn':
        ...
        logger.set_logger_dir(
            os.path.join(train_logdir, '{}-skip-{}-hist-{}-batch-{}-lr-{}-{}-eps-{}-reg-{}-{}-lamb{}-type-{}-exp-dnp-{}-ent-{}-thresh-{}-stab-{}'.format(
                ...
                0 if args.threshold == None else THRESHOLD, args.stab)))
        config = get_config()
        if args.load:

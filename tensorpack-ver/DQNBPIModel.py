...
class Model(ModelDesc):
    ...
    def _build_graph(self, inputs):
        comb_state, action, reward, isOver, action_o, old_action_o = inputs
        comb_state = tf.cast(comb_state, tf.float32)
        state = tf.slice(comb_state, [0, 0, 0, 0], [-1, -1, -1, self.channel], name='state')

        self.predict_value, pi_value, bp_value = self._get_DQN_prediction(state)
        ...
        with tf.variable_scope('target'), \
            ...
            targetQ_predict_value, target_pi_value, target_bp_value = self._get_DQN_prediction(next_state)    # NxA
        ...
        self.cost = tf.reduce_mean(q_cost + self.lamb * (pi_cost + bp_cost))

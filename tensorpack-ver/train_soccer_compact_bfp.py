    ...
    def _get_DQN_prediction(self, image):
        ...
        with tf.variable_scope('network'):
            with argscope(Conv2D, nl=PReLU.symbolic_function, use_bias=True), \
                ...
                q_l = FullyConnected('fc0', s_l, 512, nl=LeakyReLU)
                p_l = FullyConnected('p-fc0', s_l, 512, nl=LeakyReLU)
        ...
        bp_y = FullyConnected('bp-fc0', p_l, self.num_actions, nl=tf.identity)
        pi_y = FullyConnected('pi-fc0', p_l, self.num_actions, nl=tf.identity)
        fp_conc = tf.concat([Q, bp_y, pi_y, p_l], axis=1)
        fp_y = FullyConnected('fp-fc0', fp_conc, self.num_actions, nl=tf.identity)

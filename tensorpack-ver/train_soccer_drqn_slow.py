...
def get_player(viz=False, train=False):
    ...
    pl = SoccerPlayer(image_shape=IMAGE_SIZE[::-1], viz=viz, frame_skip=ACTION_REPEAT, field=FIELD, \
            ai_frame_skip=AI_SKIP, team_size=2 if MULTI_TASK else 1, mode=MODE)
    if not train:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mt', help='multi task', type=int, required=True)
    ...
    if args.task != 'train':
        ...
        logger.set_logger_dir(
            os.path.join(args.log, '{}-DRQN-slow-field-{}-skip-{}-ai_skip-{}-hist-{}-batch-{}-lr-{}-{}'.format(
                scenario, args.field, args.skip, args.ai_skip, args.hist_len, args.batch_size, args.lr, os.path.basename('soccer').split('.')[0])))
        config = get_config()
        if args.load:

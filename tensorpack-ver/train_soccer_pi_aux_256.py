...
def get_rnn_cell():
    if RNN_CELL == 'gru':
        return tf.nn.rnn_cell.GRUCell(num_units=256)
    elif RNN_CELL == 'lstm':
        return tf.nn.rnn_cell.LSTMCell(num_units=256, state_is_tuple=True)
    else:
        assert 0
    ...
    def _get_DQN_prediction(self, image):
        ...
        with tf.variable_scope('q'):
            with argscope(Conv2D, nl=PReLU.symbolic_function, use_bias=True), \
                ...
                q_l = FullyConnected('fc0-q', h, 256, nl=LeakyReLU)
                pi_l = FullyConnected('fc0-pi', h, 256, nl=LeakyReLU)
                ...
                if USE_RNN:
                    ...
                    q_l = q_l[:, -1, :]
                    # pi
                    pi_l = tf.reshape(pi_l, [self.batch_size, self.channel, 256])
...
if __name__ == '__main__':
    ...
    if args.task != 'train':
        ...
        logger.set_logger_dir(
            os.path.join('train_log', '{}-256-aux-field-{}-skip-{}-ai_skip-{}-hist-{}-batch-{}-lr-{}-lamb-{}-{}'.format(

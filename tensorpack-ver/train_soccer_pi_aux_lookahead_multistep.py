...
from DQNPIModel_multistep import Model as DQNModel
import common
from common import play_model, Evaluator, eval_model_multithread
from soccer_env import SoccerPlayer
from augment_expreplay_lookahead import AugmentExpReplay
from tensorpack.tfutils import symbolic_functions as symbf
...
def get_rnn_cell(num_units=RNN_HIDDEN):
    if RNN_CELL == 'gru':
        return tf.nn.rnn_cell.GRUCell(num_units=num_units, activation=get_activation())
    elif RNN_CELL == 'lstm':
        return tf.nn.rnn_cell.LSTMCell(num_units=num_units, state_is_tuple=True, activation=get_activation())
    else:
        assert 0
...
class Model(DQNModel):
    def __init__(self):
        super(Model, self).__init__(IMAGE_SIZE, FRAME_HISTORY, METHOD, 
            NUM_ACTIONS, GAMMA, LR, LAMB, KEEP_STATE, RNN_HIDDEN, RNN_STEP, MULTI_TASK, 3 if MULTI_TASK else 1, NUM_LOOKAHEAD, USE_REG)
    ...
    def _get_DQN_prediction(self, image):
        ...
        for i in range(self.num_agents):
            if self.num_lookahead > 0:
                assert USE_RNN
                assert RNN_STEP == 1
                pi_seq_cell = get_rnn_cell()
                pi_l_repeat = RepeatVector(self.num_lookahead + RNN_STEP)(pi_l)
                pi_seq_l, pi_seq_rnn_state_out = tf.nn.dynamic_rnn(inputs=pi_l_repeat, 
                            cell=pi_cell,
                            initial_state=pi_rnn_state_out,
                            dtype=tf.float32, scope='rnn-pi-seq-%d' % i)  
                pi_seq_l = tf.reshape(pi_seq_l, (self.batch_size * (RNN_STEP + self.num_lookahead), RNN_HIDDEN))
                pi_ys.append(FullyConnected('fct-seq-%d' % i, pi_seq_l, self.num_actions, nl=tf.identity))
            else:
                pi_seq_l = pi_l                   
                pi_ys.append(FullyConnected('fct-%d' % i, pi_seq_l, self.num_actions, nl=tf.identity))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_lookahead', help='lookahead', type=int, default=0)

    ...
    def _get_DQN_prediction(self, image):
        ...
        with tf.variable_scope('q'):
            with argscope(Conv2D, nl=PReLU.symbolic_function, use_bias=True), \
                ...
                h = (LinearWrap(image)
                     # Nature architecture
                     .Conv2D('conv0', out_channel=32, kernel_shape=8, stride=1)
                     .Conv2D('conv1', out_channel=64, kernel_shape=4, stride=1)
                     .Conv2D('conv2', out_channel=64, kernel_shape=3)())                    
                     #.FullyConnected('fc0', 512, nl=LeakyReLU)())
                l = FullyConnected('fc0', h, 512, nl=LeakyReLU)

        pi_y = FullyConnected('fcp', h, self.num_actions, nl=tf.identity)

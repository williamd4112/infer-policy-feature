...
class Model(DQNModel):
    ...
    def _get_DQN_prediction(self, image):
        ...
        with tf.variable_scope('pi'):
            with argscope(Conv2D, nl=PReLU.symbolic_function, use_bias=True), \
                    argscope(LeakyReLU, alpha=0.01):
                pi_h = (LinearWrap(image)
                     # Nature architecture
                     .Conv2D('conv0', out_channel=64, kernel_shape=6, stride=2, padding='VALID')
                     .Conv2D('conv1', out_channel=64, kernel_shape=6, stride=2, padding='SAME')
                     .Conv2D('conv2', out_channel=64, kernel_shape=6, stride=2, padding='SAME')
                     .FullyConnected('fc0', 1024, nl=LeakyReLU)
                     .FullyConnected('fc1', 512, nl=LeakyReLU)())
...
if __name__ == '__main__':
    ...
    if args.task != 'train':
        ...
        logger.set_logger_dir(
            os.path.join('train_log', 'DQNPI-relu-field-{}-skip-{}-ai_skip-{}-hist-{}-batch-{}-lr-{}-lamb-{}-{}'.format(

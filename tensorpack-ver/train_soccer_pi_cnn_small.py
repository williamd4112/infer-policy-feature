...
class Model(DQNModel):
    ...
    def _get_DQN_prediction(self, image):
        ...
        with argscope(Conv2D, nl=PReLU.symbolic_function, use_bias=True), \
                argscope(LeakyReLU, alpha=0.01):
            pi_l = Conv2D('pi-conv0', image, out_channel=32, kernel_shape=5)
            pi_l = MaxPooling('pi-pool0', pi_l, 2)
            pi_l = Conv2D('pi-conv1', pi_l, out_channel=32, kernel_shape=5)
            pi_l = MaxPooling('pi-pool1', pi_l, 2)
            pi_l = Conv2D('pi-conv2', pi_l, out_channel=64, kernel_shape=4)
            pi_l = MaxPooling('pi-pool2', pi_l, 2)
            pi_l = Conv2D('pi-conv3', pi_l, out_channel=64, kernel_shape=3)
            pi_h = FullyConnected('pi-fc0', pi_l, 512, nl=tf.nn.relu)
        pi_y = FullyConnected('pi-fc1', pi_h, self.num_actions, nl=tf.identity)
